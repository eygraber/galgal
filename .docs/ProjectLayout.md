...
Because of how the scoping is set up in [Galgal DI] all of the graphs are created in the `app` module, so that is the only
...
```mermaid
graph TD;
    app{{<span style='text-decoration:underline'>app</span><br/>GalgalApplication<br/>GalgalActivity}};
