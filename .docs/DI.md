...
```mermaid
graph TD;
    GalgalApplicationComponent-->GalgalActivityComponent-->GalgalNavComponent-->*DestinationComponent
```

`GalgalApplicationComponent` is the root, and uses [MergeComponent] to include all of the modules contributed to the `AppScope`.

`GalgalActivity` is a [SubComponent] of `GalgalApplicationComponent`, and includes all of the modules contributed to the `ActivityScope`.

`GalgalNavComponent` is a [SubComponent] of `GalgalActivityComponent`, and includes all of the modules contributed to the `NavScope`.
In practice, this only exists so that `GalgalActivityComponent` can remain isolated in the `app` module. Otherwise `GalgalActivityComponent`
would have to be moved into its own module, so that both the `app` and `nav` modules can access it.

Each `*DestinationComponent` is a [SubComponent] of `GalgalNavComponent`, and has a 1:1 relationship with its [Destination].
It is used to create the `ViceDestination` in `GalgalNav`.
...
// the instance of Foo will be cached in the scoped Component, e.g. in `GalgalActivityComponent`
@Inject
