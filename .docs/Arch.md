...
Galgal's Android architecture is inspired by the [Android App Architecture],
...
`GalgalApplication` is a custom `Application` that holds the [DI AppComponent] instance,
and any needed system hooks. Any initialization needed should be delegated to `GalgalInitializer`.

### Activity

`GalgalActivity` is the sole `Activity` in the project which exists to:
...
Screens in the app are built using [VICE], and define a `GalgalDestination` along with all of the required
VICE components (`ViceView`, `Intent`, `ViceCompositor`, `ViceEffects`, and `ViewState`).

Each `GalgalDestination` defines and holds a [DI DestinationComponent].
